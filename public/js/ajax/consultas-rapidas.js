...
function clickDocenteItem( cedDoc ){
  ...
  if ( $("#formHorarioSeccion input[name=tipo_consulta]:checked").val() == "CD" ) {
    ...
    localStorage.setItem( "consulta" , "consultar-disponibilidad-docente" )
...
function clickSeccionItem( codSec , turno ){
      localStorage.setItem( "seccion_seleccionada" , JSON.stringify( codSec ) )
    localStorage.setItem( "consulta" , "consultar-disponibilidad-seccion" )
...
function clickAmbienteItem( codAmb ){
  if ( $("#formHorarioSeccion input[name=tipo_consulta]:checked").val() == "CD" ) {
    localStorage.setItem( "ambiente_seleccionado" , JSON.stringify( codAmb ) )
    localStorage.setItem( "consulta" , "consultar-disponibilidad-ambiente" )
    $(".plantilla table tr td.bloque-de-hora")
    .append(`<div class="switch">
              <label>
                <input 
  ...
  if ( $("input[name=switchConsultarBloques]:checked").length == 0 ){
    ...
    var consulta = localStorage.getItem( 'consulta' ) 
      ...
      $.ajax({           
          ...
          data: {
            "codSec" : JSON.parse( localStorage.getItem( 'seccion_seleccionada' )),
      ...
      $.ajax({           
          ...
          url:'?controlador=horariosAmbientes&actividad=consultar',
          data: {
            "codAmb" : JSON.parse( localStorage.getItem( 'ambiente_seleccionado' ) )
          }
      })
      .done(function(respuesta){
      ...
      $.ajax({           
          ...
          url:'?controlador=horariosDocentes&actividad=consultar',
          data: {
            "cedDoc" : JSON.parse( localStorage.getItem( 'docente_seleccionado' ) )
          }
      })
      .done(function(respuesta){
          var arrayBloquesChecked = []
